...
(function () {
  ...
  function openPlaylistPanel() {
    document.querySelector('.playlist-overlay')?.remove();
    const list = getPlaylist();
  ...
  function openFavoritesPanel() {
    document.querySelector('.favorites-overlay')?.remove();
    document.querySelector('.playlist-overlay')?.remove();
  ...
  function renderHome() {
    ...
    root.querySelectorAll('.copy-btn').forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        navigator.clipboard?.writeText(text).then(() => showToast('Quote copied')).catch(() => showToast('Could not copy'));
      });
    });
    wireFavButtons(root);
  }
  ...
  function wirePoemNav(container, poem) {
    ...
    const n = poemNavNeighbors(poem.id);
    if (n) {
      block.querySelector('[data-poem-adj="prev"]')?.addEventListener('click', () => navigate(`poems/${n.prevId}`));
      block.querySelector('[data-poem-adj="next"]')?.addEventListener('click', () => navigate(`poems/${n.nextId}`));
    }
  ...
  function renderQuoteDetail(id) {
    ...
    root.querySelectorAll('.copy-btn').forEach(btn => {
      btn.addEventListener('click', () => {
        navigator.clipboard?.writeText(btn.dataset.text).then(() => showToast('Quote copied')).catch(() => showToast('Could not copy'));
      });
    });
    if (n) {
